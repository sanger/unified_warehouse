...
class AddCherrypickedSamplesView < ActiveRecord::Migration[6.0]
  def up
    ...
    execute <<-SQL.squish
      ...
      SELECT root_sample_id, plate_barcode, phenotype, coordinate, created, event_type
      FROM (
        ...
        mlwh_stock_resource.labware_coordinate AS "coordinate", mlwh_sample.created AS "created", mlwh_events_event_types.key AS "event_type"
        ...
        mlwh_lh_sample.coordinate AS "coordinate", mlwh_sample.created AS "created", mlwh_events_event_types.key AS "event_type"
        FROM sample as mlwh_sample
      ...
      GROUP BY cp_sample.root_sample_id, cp_sample.plate_barcode, cp_sample.phenotype, cp_sample.coordinate, cp_sample.created, cp_sample.event_type
    SQL
  end
