  ...
  context '.create_or_update_from_json' do
    ...
    context 'without existing records' do
      ...
      context 'when the record is deleted' do
        ...
        before(:each) do
          ...
          described_class.send(:create_or_update, attributes.merge(:updated_at=>modified_at,:deleted_at => modified_at),example_lims)
        end
    ...
    context 'with an existing record' do
      ...
      context 'when the new record is not current' do
        before(:each) do
          described_class.send(:create_or_update, attributes.merge(:updated_at => modified_at - 2.hours), example_lims)
        end
        ...
        ResourceTools::IGNOREABLE_ATTRIBUTES.each do |attribute|
          ...
          context "when #{attribute.to_sym.inspect} changes" do
            before(:each) do
              ...
              described_class.send(:create_or_update, attributes.merge(:updated_at => modified_at), example_lims)
            end
